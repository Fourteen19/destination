...
class EmployerArticleSelector extends Component
{
    ...
    public function mount($label, $articleUuid, $name, $includeClientArticles)
    {
        ...
        if ($articleUuid)
        {
            $article = ContentLive::where('uuid', '=', $articleUuid)->where('template_id', 4)->select('uuid', 'title')->first()->toArray();
    ...
    {
        return view('livewire.admin.employer-article-selector');
    }
