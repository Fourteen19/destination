<div class="position-relative mb-4" x-data="{ showDropdown: @entangle('showDropdown') }" @click.away="showDropdown = false">
    ...
    <div class="position-relative" wire:key="{{ $name }}">
        <div class="text-danger error" id="{{ $name }}_error" style="display:none">Please select an article</div>
        <input
            type="text"
            name="{{ $name }}"
            id="{{ $name }}"
        ...
        @if ($selectedArticle)
            <a class="position-absolute cursor-pointer remove-selection" id="{{ $name }}_remove" wire:click="reset">
...
<script>
    ...
    article_name = "{{ $name }}";
    article_type = article_name.replace('article','type');

    if ($("input[name='"+article_type+"']:checked").val() == 'managed'){

        if ($('#'+article_name + "_remove").length)
        {
            $('#'+article_name + "_error").hide();
        } else {
            $('#'+article_name + "_error").show();
        }
