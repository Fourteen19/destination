            ...
            <div wire:sortable="updateGroupOrder" wire:sortable-group="updateVideosOrder" style="display: flex">
                <div wire:key="group-videos" wire:sortable.item="videos">
                    ...
                    <div class="rounded p-4 form-outer">
                        <ul wire:sortable-group.item-group="videos">
                            {{-- wire:sortable="updateVideoOrder" id="videos-list" wire:key="videos-list" class="drag-list" --}}
                        @foreach($relatedVideos as $key => $video)
                            {{-- <li wire:sortable.item="{{ $key }}" wire:key="video-{{ $key }}" class="drag-box"> --}}
                            <li wire:sortable-group.item="{{ $key }}" wire:key="video-{{ $key }}" class="drag-box">
                                <div class="row">
