                ...
                <div class="suggestions position-absolute" x-show="articlesSuggestionsVisible">
                    ...
                    <ul class="suggestion-results list-unstyled mb-0">
                        @foreach($searchResults as $keyword)
                            ...
                            <li wire.key="keyword_{{$loop->index}}"><a href="#" wire:click.prevent="clickKeyword('{{$keyword['name'][app()->getLocale()]}}')" class="td-no keyword-link">{{$keyword['name'][app()->getLocale()]}}</a></li>
                        @endforeach
                    </ul>
                </div>
            {{-- @else
