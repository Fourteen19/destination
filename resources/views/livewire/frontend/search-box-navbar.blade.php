            ...
            <div wire:loading wire:target="search" class="searching">Searching</div>
                <ul class="suggestion-results list-unstyled mb-0">
                    @foreach($searchResults as $keyword)
                        <li><a href="{{route('frontend.search', ['clientSubdomain' => session('client.subdomain'), 'searchTerm' => $keyword['name'][app()->getLocale()] ] )}}" class="td-no keyword-link">{{$keyword['name'][app()->getLocale()]}}</a></li>
                    @endforeach
                </ul>
