...
<div class="col-lg-3">
    <p class="t24 fw700 mb-lg-0">Search results {{ $paginator->firstItem() }} - {{ $paginator->lastItem() }} of {{ $paginator->total() }}</p>
</div>
<div class="col-lg-9">
    ...
    {{-- Pagination Elements --}}
                @foreach ($elements as $element)
                    ...
                    <div class="d-inline-block t24 fw700 mr-2" aria-disabled="true"><span class="page-link">{{ $element }}</span></div>
                    @endif
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            ...
                            <div class="d-inline-block paginate" wire:key="paginator-page-{{ $page }}" aria-current="page"><span class="page-link">{{ $page }}</span></div>
                            @else
                            <div class="d-inline-block paginate" wire:key="paginator-page-{{ $page }}"><button type="button" class="page-link" wire:click="gotoPage({{ $page }})">{{ $page }}</button></div>
                            @endif
                        @endforeach
                    @endif
                @endforeach
